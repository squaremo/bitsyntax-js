...
function get_string(segment) {
  $line("byteoffset = offset / 8;");
  var strlen = segment.value != undefined ? segment.value.length : segment.size;
  var strlenbits = strlen * 8;
  $line("offset += %d;", strlenbits);
  $line("if (offset > binsize) { return false; }");
  $line("else { result = bin.toString('utf8',byteoffset,", $("byteoffset + %d); ", strlen));
  if (segment.hex) {
