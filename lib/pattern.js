...
function variable(name, size, specifiers0) {
  ...
  add_padding_specifiers(segment, specifiers)
  return segment;
}
...
var TYPES = {'integer': 1, 'binary': 1, 'float': 1, 'string': 1, 'left': 1, 'right': 1, 'zero': 1, 'space': 1};
...
function add_padding_specifiers(segment, specifiers){
  for (var s in specifiers) {
    if (s == 'left' || s == 'right' || s == 'zero' || s == 'space') {
      segment[s] = 1;
    }
  }
  return segment;
